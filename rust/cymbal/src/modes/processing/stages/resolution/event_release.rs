...
/// content hash for mobile builds — so they get separate caches. moka caches are internally Arc'd,
/// so cloning this into each per-batch `ResolutionStage` is cheap.
#[derive(Clone)]
...
impl ReleaseCache {
    pub fn new(capacity: u64, ttl: Duration) -> Self {
        Self {
            by_id: CacheBuilder::new(capacity).time_to_live(ttl).build(),
            by_hash: CacheBuilder::new(capacity).time_to_live(ttl).build(),
        }
    }

    /// A no-op cache for the paths that never resolve event releases (the remote resolution server
    /// and frame-resolution tests). Capacity 0 means nothing is retained, but those paths never
    /// read it anyway.
    ...
    {
        let key = (team_id, id);
        if let Some(hit) = self.by_id.get(&key).await {
            metrics::counter!(ANCILLARY_CACHE, "type" => "release_by_id", "outcome" => "hit")
                .increment(1);
            return Ok(hit);
        }
        metrics::counter!(ANCILLARY_CACHE, "type" => "release_by_id", "outcome" => "miss")
            .increment(1);
        let record = ReleaseRecord::for_id(e, id, team_id).await?;
        self.by_id.insert(key, record.clone()).await;
        Ok(record)
    }
    ...
    where
        E: Executor<'c, Database = Postgres>,
    {
        let key = (team_id, hash_id.to_string());
        if let Some(hit) = self.by_hash.get(&key).await {
            metrics::counter!(ANCILLARY_CACHE, "type" => "release_by_hash", "outcome" => "hit")
                .increment(1);
            return Ok(hit);
        }
        metrics::counter!(ANCILLARY_CACHE, "type" => "release_by_hash", "outcome" => "miss")
            .increment(1);
        let record = ReleaseRecord::for_hash(e, hash_id, team_id).await?;
        self.by_hash.insert(key, record.clone()).await;
        Ok(record)
    }
}
