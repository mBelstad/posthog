...
export function useTaxonomicFilter(opts: UseTaxonomicFilterOptions): TaxonomicFilterApi {
    ...
    const effectiveExcludedProperties = useMemo<ExcludedProperties>(() => {
        ...
        for (const displayGroup of groups) {
            ...
            if (!sourceGroup?.excludedProperties?.length) {
                continue
            }
            merged[sourceGroup.type] = Array.from(
                new Set([...(merged[sourceGroup.type] ?? []), ...sourceGroup.excludedProperties])
            )
        }
        return merged
    ...
    const getLocalOverride = useTaxonomicLocalOverrides({
        ...
        excludedOperators,
        selectingKeyOnly,
        excludedProperties: effectiveExcludedProperties,
    })
